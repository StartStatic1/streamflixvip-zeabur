...
// CORREÇÃO 1: Detecta "Título SxxEyy" (sem espaço) e "Título Sxx Eyy" (com espaço)
// O servidor SOS envia episódios com espaço (ex: "Orange Is the New Black S01 E01"),
// então a regex precisa permitir o espaço opcional (\s*) entre a temporada (S) e o episódio (E).
const SEASON_EP_RE = /^(.*?)\s+S(\d{1,2})\s*E(\d{1,4})\s*$/i;

// Categorias que consideramos "TV ao vivo" (heurística por group-title)
// CORREÇÃO 2: Filmes/séries usam "Filmes | Categoria" (com pipe) OU "FILMES: Categoria" (com dois pontos)
// O servidor SOS usa "FILMES: AÇÃO" (dois pontos), então a regex precisa aceitar ambos [:|]
function classify(entry) {
  ...
  // "Filmes | Categoria" ou "Filmes: Categoria" (pipe OU dois pontos) = subcategoria de filme individual.
  ...
  // Se tiver ano E estiver em grupo de filmes/streaming OU na URL de VOD (/movie/), é filme
  ...
  // CORREÇÃO 3: Fallback extra de URL /movie/ APENAS SE estiver em um grupo reconhecido de filme/streaming
  // Se não tiver ano no nome (ex: "2ª Rodada Turquia x Paraguai"), mas a URL tem /movie/, 
  // ele NUNCA deve ser classificado como filme se o grupo for "COPA 26 - CAZE" ou "ESPORTES".
  if (url.includes('/movie/') && !yearMatch && (looksLikeMovieGroup || looksLikeStreamingGroup)) {
    return { kind: 'movie', title: name.trim(), year: null };
  }

  // Sem ano, sem SxxExx e não está em grupo de filme -> assume canal ao vivo
  // Isso impede que jogos de futebol e programas de TV entrem no catálogo de filmes só pela URL
  return { kind: 'live' };
}
