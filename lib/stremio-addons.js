...
function streamToSource(stream, addonName) {
  ...
  const title =
    stream.title || stream.name || stream.description || stream.quality || 'Stream';
  const short = String(title).replace(/\s+/g, ' ').trim().slice(0, 48);
  return {
    source_url: url.trim(),
    source_label: `Addon · ${addonName}${short ? ' · ' + short : ''}`,
    priority: 5,
  };
}
