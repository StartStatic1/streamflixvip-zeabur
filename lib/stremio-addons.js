...
const COLLECT_DEADLINE_MS = 9000;
...
function isHttpStreamUrl(u) {
  ...
  // Comet/torrentio costumam mandar pagina ou endpoint de info sem midia
...
function shortAddonName(name) {
  ...
  if (/comet/i.test(n)) return 'Comet';
...
function streamToSource(stream, addonName) {
  // Stremio: url direta OU infoHash/magnet (torrent) — so aceitamos url HTTP
...
async function collectAddonSources(serviceKey, tmdbId, mediaType, season, episode) {
  ...
  // Cada add-on em paralelo, mas com teto global — series nao ficam 30s+ no spinner
