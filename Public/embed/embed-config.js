...
async function handleConfig(req, res) {
  ...
  try {
    ...
    // Origin identificável, o que acontece com frequência em iframes), a
    // condição inteira dava false e o domínio configurado era simplesmente
    // ignorado, liberando o embed em QUALQUER host. Agora: domínio
    // configurado + não foi possível identificar de onde veio a requisição
    // = bloqueia (mais seguro travar quando não dá pra checar do que
    // liberar por padrão).
    if (partner.domain) {
      if (!refererHost) {
