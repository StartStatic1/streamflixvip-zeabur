    ...
    fun loadDetails() {
        viewModelScope.launch {
            ...
            try {
                ...
                if (mediaType == "tv") {
                    // Season 0 = Especiais (OVAs). T1–Tn = fluxo normal.
                    // Nunca abrir Specials por padrão — só deep-link.
                    val seasons = details.seasons.orEmpty().filter { it.season_number >= 0 }
                    val regular = seasons.filter { it.season_number >= 1 }
                    val seasonToOpen = when {
                        initialSeason >= 0 -> initialSeason
                        regular.isNotEmpty() -> regular.first().season_number
                        else -> seasons.firstOrNull()?.season_number
                    }
                    if (seasonToOpen != null) {
                        expandSeason(seasonToOpen)
                    }
                    if (initialSeason >= 0) {
                        preloadEpisodeSourcesSilently(initialSeason, initialEpisode.coerceAtLeast(0))
                    }
                }
    ...
    fun expandSeason(season: Int) {
        ...
        viewModelScope.launch {
            ...
            if (stillCurrent.expandedSeason == season) {
                // Season 0: só EPs cadastrados no painel (sem lista TMDB inteira)
                val eps = if (season == 0 && withSources != null) {
                    stillCurrent.episodesOfExpandedSeason.filter { it.episode_number in withSources }
                } else {
                    stillCurrent.episodesOfExpandedSeason
                }
                _uiState.value = stillCurrent.copy(
                    episodesWithSources = withSources,
                    episodesOfExpandedSeason = eps,
                )
            }
        }
    }
