...
data class VipSource(
    ...
    val isDirectPlayable: Boolean
        get() = urlLooksDirectPlayable(source_url)
...
fun urlLooksDirectPlayable(url: String): Boolean {
    ...
    val embedHints = listOf(
        "/embed/", "megaembed", "filemoon", "mixdrop", "streamtape",
        "doodstream", "voe.sx", "streamwish", "player.html",
    )
    if (embedHints.any { lower.contains(it) }) return false
    if (path.endsWith(".m3u8") || path.endsWith(".mp4") || path.endsWith(".mkv") ||
        path.endsWith(".webm") || path.endsWith(".m4v") || path.endsWith(".mov") ||
        path.endsWith(".ts") || path.endsWith(".m2ts") || path.endsWith(".avi")) {
        return true
    }
    if (lower.contains("/stream-proxy") ||
        lower.contains("tb-cdn") ||
        lower.contains("torbox") ||
        lower.contains("/dld/") ||
        lower.contains("requestdl") ||
